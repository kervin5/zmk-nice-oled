...
#if !IS_ENABLED(CONFIG_NICE_OLED_WIDGET_PROFILE_BIG)
static void draw_inactive_profiles(lv_obj_t *canvas, const struct nice_oled_central_state *state) {
...
#if !IS_ENABLED(CONFIG_NICE_EPAPER_ON)
static void draw_active_profile_text(lv_obj_t *canvas, const struct nice_oled_central_state *state) {
    // new label_dsc
